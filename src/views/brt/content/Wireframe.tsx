...
const Wireframe = React.forwardRef(
  (
    ...
    const [removable_dot_arr, setRemovableDotArr] = React.useState<number[]>(
      []
    );
    ...
    const updateRecommendList = (
      ...
      if (action === 'remove') {
        ...
        let temp_desc: string;
        temp_desc = temp[is_exist].description
          .replace(description, '')
          .replaceAll('  ', ' ');
        if (temp_desc[0] === ' ')
          temp_desc = temp_desc.slice(1, temp_desc.length);

        if (!temp_desc) {
          temp.splice(is_exist, 1);
        } else if (temp_desc.search(']') === -1) {
          if (temp_desc.search('upper_teeth') >= 0) {
            temp_desc = 'upper_teeth';
          } else if (temp_desc.search('lower_teeth') >= 0) {
            temp_desc = 'lower_teeth';
          }

          temp[is_exist].description = temp_desc;
        }
    ...
    const setUpperTeeth = () => {
      ...
      const will_delete_arr: number[] = [];
      let will_locked: boolean = false;

      for (const brush of recommended_list) {
        if (brush.id !== current_brush.id) {
          const dot_el_arr = brush.description.split(' ');

          let temp_desc = brush.description;

          for (const dot of dot_el_arr) {
            if (dot === 'lower_row') {
              will_locked = true;
            }
            if (dot.search(']') >= 0) {
              ...
              if (highlight.upperTeeth.includes(temp)) {
                temp_desc = temp_desc.replace(dot, '').replace('  ', ' ');
                if (temp_desc[0] === ' ')
                  temp_desc = temp_desc.slice(1, temp_desc.length);
              }
            }
          ...
          const find_index = recommended_list.findIndex(
            item => item.id === brush.id
          );

          if (!temp_desc) {
            will_delete_arr.push(find_index);
          } else {
            if (temp_desc.search('lower_row') >= 0) {
              temp_desc = 'lower_row';
            }
            updateRecommendDesc(find_index, temp_desc);
          }
        }
      }
      ...
      setRemovableDotArr([...will_delete_arr, ...removable_dot_arr]);
        ...
        if (brush.id !== current_brush.id) {
          ...
          for (const dot of dot_el_arr) {
            ...
            if (dot.search(']') >= 0) {
              ...
              if (highlight.lowerTeeth.includes(temp)) {
                temp_desc = temp_desc.replace(dot, '').replace('  ', ' ');
                if (temp_desc[0] === ' ')
                  temp_desc = temp_desc.slice(1, temp_desc.length);
              }
          ...
          if (!temp_desc) {
            ...
            if (temp_desc.search('upper_row') >= 0) {
              temp_desc = 'upper_row';
            }
            updateRecommendDesc(find_index, temp_desc);
          }
    ...
    const handlePointerEvents = () => {
      ...
      for (const temp_recommended_item of recommended_list) {
        if (
          temp_recommended_item.description ===
          wireframe_constant.recommend_desc.upper_row
        ) {
          setSelectedTeeth(wireframe_constant.selected_teeth.upper_teeth);
          break;
      ...
      if (
        ...
        if (el) {
          ...
          if (el.style.visibility === 'visible')
            ...
            updateRecommendList(current_brush, 'remove', `[${display_number}]`);
        }
      }
      if (
    ...
    }, [active_brush, active_step, selected_opt]);
    ...
    React.useEffect(() => {
      if (removable_dot_arr.length > 0) {
        const temp: number | undefined = removable_dot_arr.pop();
        if (temp !== undefined) {
          setRemovableDotArr([...removable_dot_arr]);
          deleteBrush(temp);
        }
      }
      // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      <>
        ...
        <Box
          ...
          sx={{
            ...
            pointerEvents:
              !current_brush?.hex || active_step === 4 ? 'none' : 'all',
          }}
        >
          {isLoaded && (
            <>
        ...
        <Grid container direction="row" justifyContent="space-between">
          <Grid item xs={1} className="middle-left noselect">
            <Typography
              ...
              sx={{
                opacity:
                  (!current_brush?.hex || active_step === 0) && active_step < 2
                    ? '0.5'
                    : '1',
              }}
            >
